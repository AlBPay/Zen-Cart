...
flog( 'Alliance bank ITN call received' );
...
if ( ! $fError ) {
	...
	flog( 'Alliance bank Data: ' . print_r( $fData, true ) );
...
if ( ! $fError ) {
	...
	switch ( $txnType ) {
		...
		case 'new':
			...
			// Create Fondy order
			flog( 'Creating Alliance bank order' );
			...
			flog( 'Creating Alliance bank payment status history record' );
			...
			$sqlArray = array(
				...
				'customer_notified' => '0',
				'comments'          => 'Alliance bank id: ' . $fData['payment_id'],
			);
			...
			if ( F_DEBUG ) {
				$subject = "Alliance bank ITN on your site";
				$body    =
					"Hi,\n\n" .
					"A Alliance bank transaction has been completed on your website\n" .
					"------------------------------------------------------------\n" .
					"Site: " . STORE_NAME . " (" . HTTP_SERVER . DIR_WS_CATALOG . ")\n" .
					"Order ID: " . $zcOrderId . "\n" .
					//"User ID: ". $db->f( 'user_id' ) ."\n".
					"Alliance bank Transaction ID: " . $fData['f_payment_id'] . "\n" .
					"Alliance bank Payment Status: " . $fData['payment_status'] . "\n" .
		...
		case 'failed':
			...
			$comments = 'Payment failed (Alliance bank id = ' . $fData['f_payment_id'] . ')';
			$sqlArray = f_createOrderHistoryArray( $fData, $fOrderId, $ts );
			zen_db_perform( TABLE_FONDY_PAYMENT_STATUS_HISTORY, $sqlArray );
			...
			// Sending email to admin
			$subject = "Alliance bank ITN Transaction on your site";
			$body    =
				"Hi,\n\n" .
				"A failed Alliance bank transaction on your website requires attention\n" .
				"------------------------------------------------------------\n" .
				"Site: " . STORE_NAME . " (" . HTTP_SERVER . DIR_WS_CATALOG . ")\n" .
				"Order ID: " . $zcOrderId . "\n" .
				//"User ID: ". $db->f( 'user_id' ) ."\n".
...
if ( $fError ) {
	...
	$subject = "Alliance bank ITN error: " . $fErrMsg;
	$body    =
		"Hi,\n\n" .
		"An invalid Alliance bank transaction on your website requires attention\n" .
		"------------------------------------------------------------\n" .
		"Site: " . STORE_NAME . " (" . HTTP_SERVER . DIR_WS_CATALOG . ")\n" .
	...
	if ( isset( $fData['f_payment_id'] ) ) {
		$body .= "Alliance bank Transaction ID: " . $fData['f_payment_id'] . "\n";
	}
	if ( isset( $fData['payment_status'] ) ) {
		$body .= "Alliance bank Payment Status: " . $fData['payment_status'] . "\n";
	}
	$body .=
		"\nError: " . $fErrMsg . "\n";
